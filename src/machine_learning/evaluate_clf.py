...
def confusion_matrix_and_report(X, y, pipeline, label_map, threshold=0.5):
    """
    Print a styled confusion matrix and classification report for one split.

    Uses predict_proba so the threshold can be tuned independently of the
    model — important when the business requirement demands high recall.
    ...
    Args:
        ...
        threshold (float): Probability cutoff for the positive class. Default 0.5.
    """
...
def clf_performance(X_train, y_train, X_val, y_val, X_test, y_test,
    ...
    Prints the confusion matrix and classification report for each split,
    making it easy to spot overfitting by comparing train vs val vs test.
