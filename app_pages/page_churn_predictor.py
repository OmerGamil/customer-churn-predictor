...
predicts churn probability using the saved ML pipeline and optimal
threshold. Addresses Business Requirement 2 of the Customer Churn
Predictor project.
"""
...
def page_churn_predictor_body():
    """Render the Churn Predictor page with input form and prediction."""
    st.write("## Churn Predictor")
    st.info(
        "Enter customer details below and click **Predict** to find out "
    ...
    with col3:
        ...
        paperless_billing = st.selectbox(
            "Paperless Billing", options=["No", "Yes"]
        )
        ...
        monthly_charges = st.slider(
            "Monthly Charges ($)",
            min_value=18.0, max_value=119.0, value=65.0
        )
        total_charges = st.slider(
            "Total Charges ($)",
            min_value=18.0, max_value=8685.0, value=1500.0
        )
        ...
        try:
            ...
            if prediction_label == 1:
                st.error(
                    ...
                    " \n\n"
                    "We recommend proactive retention action such as a "
                    "personalised offer or a customer satisfaction "
                    "check-in."
                )
            else:
                st.success(
                    "**Prediction: This customer is not likely to "
                    "churn.** \n\n"
            ...
            st.warning(
                ...
                "Please run Notebook 05_ModellingEvaluation.ipynb to "
